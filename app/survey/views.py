...
survey_logic = SurveyLogicProvider('orm')
...
def create_survey():
    if request.method == 'POST':
        ...
        return redirect(
            url_for('survey.add_option', survey_id=survey['id'])
        )
...
def submit_vote():
    ...
    if not option_ids:
        ...
        return redirect(
            url_for('survey.get_survey', survey_id=survey_id)
        )
...
def get_votes_for_survey(survey_id):
    data = survey_logic.get_votes_for_survey(survey_id)
    return render_template(
        'votes.html',
        votes=data['votes'],
        survey=data['survey']
...
def add_option(survey_id):
    if request.method == 'POST':
        ...
        return redirect(
            url_for('survey.add_option', survey_id=survey_id)
        )
