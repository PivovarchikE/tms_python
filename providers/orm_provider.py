...
class OrmProvider(DataProvider):
    ...
    def connect():
        if OrmProvider.session:
            ...
            print('Create existing ORM connection connection')
    ...
    def get_books_by_genre(genre_name):
        with OrmProvider.connect() as session:
            books = (
                ...
                .join(models.Genre)
                .filter(models.Genre.genre_name.ilike(f"%{genre_name}%"))
                .all()
                     )
    ...
    def get_books_by_author_and_year(author_name, year):
        with OrmProvider.connect() as session:
            books = (
                ...
                .filter(models.Author.author_name.ilike(f"%{author_name}%"))
                .all()
                     )
            return [
                (book.title, book.author.author_name, book.publication_year)
                for book in books
            ]
